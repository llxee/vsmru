...
function Get-VSMRU {
    ...
    process {
        ...
        if ($vsList.Count -eq 0) { Write-Error "No Visual Studio installation detected."; return }
        ...
        if ($Delete) {
            ...
            if (-not [int]::TryParse($userInput, [ref]$null)) {
                Write-Host "Invalid input. Cancelled."; return
            }
            $selIdx = [int]$userInput
            $target = $allEntries | Where-Object { $_.DisplayIndex -eq $selIdx }
            if (-not $target) {
                Write-Host "Index $selIdx not found. Cancelled."; return
            }
            $conf = Read-Host "Delete '$($target.Name)'? [y/N]"
            if ($conf -ne 'y') { Write-Host "Cancelled."; return }
            if ($isRegistry) {
            ...
            Show-Entries -Entries $allEntries -MaxCount $Count
        }
    }
}
...
New-Alias -Name vsmru -Value Get-VSMRU -Scope Global -Force

Export-ModuleMember -Function Get-VSMRU -Alias vsmru
